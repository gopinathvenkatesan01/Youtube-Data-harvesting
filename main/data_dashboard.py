...
def yt_dash_board(channel_name,channel_info,video_df,comment_dt):
    ...
    with scol3:
        ...
        if "video_name" not in st.session_state:
            st.session_state.vid_name = None

        # Define a placeholder variable to store the selected value temporarily
        selected_vid_name = st.session_state.vid_name

        # Use on_change event handler to update the session state
        selected_vid_name = st.selectbox(" :orange[Choose Your Video Name]", vid_names, key="video_name")

        # Use the stored value
        st.session_state.vid_name = selected_vid_name
        
            
        selected_vid = video_df.loc[video_df['Video_Name'] == st.session_state.vid_name, 'Video_Id'].to_list()[0]
